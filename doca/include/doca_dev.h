 ...
 *     or dev was created by doca_dev_open_from_pd()
 ...
 *
 * @details The PCI address string format is "Domain:Bus:Device.Function", e.g., "0000:83:00.0".
 *
 * @param [in] devinfo
 * The device to query.
 ...
 */
__DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_get_is_pci_addr_equal(const struct doca_devinfo *devinfo,
...
/**
 * @brief Get the mmap export to DPU capability of the device.
 *
 * @details Get uint8_t value defining if the device can be used to export an mmap.
 * See doca_mmap_export_dpu() in doca_mmap.h
...
__DOCA_EXPERIMENTAL
doca_error_t
doca_devinfo_get_is_mmap_export_dpu_supported(const struct doca_devinfo *devinfo,
					      uint8_t *mmap_export);

/**
 * @brief Get the mmap create from export DPU capability of the device.
 *
 * @details Get uint8_t value defining if the device can be used to create an mmap from an exported mmap where
 * the exported mmap was created using doca_mmap_export_dpu().
...
__DOCA_EXPERIMENTAL
doca_error_t
doca_devinfo_get_is_mmap_from_export_dpu_supported(const struct doca_devinfo *devinfo,
						   uint8_t *from_export);
 ...
 * See doca_devinfo_rep_list_create().
 * true  - device can be used with the remote list create API with filter DOCA_DEV_REP_FILTER_EMULATED.
 * false - providing DOCA_DEV_REP_FILTER_EMULATED is guaranteed to fail with DOCA_ERROR_NOT_SUPPORTED.
 *
 * @param [in] devinfo
 * The device to query.
 * @param [out] emulated_supported
 ...
 */
__DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_is_list_emulated_supported(const struct doca_devinfo *devinfo,
							      uint8_t *emulated_supported);
 ...
 */
__DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_is_pci_addr_equal(const struct doca_devinfo_rep *devinfo_rep,
						const char *pci_addr_str, uint8_t *is_equal);
