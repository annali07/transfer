...
enum doca_apsh_netscan_attr {
	DOCA_APSH_NETSCAN_PID,					/**< netscan process id */
	DOCA_APSH_NETSCAN_COMM,					/**< netscan process name */
	...
	DOCA_APSH_NETSCAN_TIME,					/**< netscan connection creation time */
};
...
enum doca_apsh_yara_rule {
	DOCA_APSH_YARA_RULE_HELLO_WORLD,			/**< yara rule that scans for "Hello World". Rule name is
								  *  "Hello_World". */
	DOCA_APSH_YARA_RULE_REFLECTIVE_DLL_INJECTION,		/**< yara rule that scans for Reflective Dll Injection
								  *  attack. Rule name is "Reflective_Dll_Injection". */
	DOCA_APSH_YARA_RULE_MIMIKATZ,				/**< yara rule that scans for Mimiaktz process running on
