...
enum doca_regex_status_flag {
	/* Job processing exceeded the maximum number of primary threads */
	DOCA_REGEX_STATUS_MAX_PRI_THREADS = 1ULL << 0,
	/* Job processing exceeded the maximum number of secondary threads */
	DOCA_REGEX_STATUS_MAX_SEC_THREADS = 1ULL << 1,
	/* Job processing exceeded the maximum time limit */
	DOCA_REGEX_STATUS_MAX_LATENCY = 1ULL << 2,
	/* Job processing exceeded the maximum number of matches */
	DOCA_REGEX_STATUS_MAX_MATCH = 1ULL << 3,
	/* Job processing exceeded the maximum number of prefixes */
	DOCA_REGEX_STATUS_MAX_PREFIX = 1ULL << 4,
	/* CQE failure, some aspect of the job was incorrect */
...
#define DOCA_REGEX_STATUS_SEARCH_FAILED \
	(DOCA_REGEX_STATUS_MAX_PRI_THREADS | DOCA_REGEX_STATUS_MAX_SEC_THREADS | \
	 DOCA_REGEX_STATUS_MAX_LATENCY | DOCA_REGEX_STATUS_MAX_MATCH | \
	 DOCA_REGEX_STATUS_MAX_PREFIX | DOCA_REGEX_STATUS_CQE_FAILURE_INVALID_JOB)
 ...
 *
 * @note Enabling in order reponses may negatively impact performance as the hardware device may process some jobs
 * quicker than others but software will not have access to these responses until previous jobs in the queue are
 * completed. This feature is disabled by default and is recommended to only be enabled if it is a hard requirement
 ...
 */
__DOCA_EXPERIMENTAL
doca_error_t doca_regex_set_in_order_responses_enabled(struct doca_regex *regex, bool enabled);
