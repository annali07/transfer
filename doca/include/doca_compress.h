...
struct doca_compress_lz4_job {
	...
	struct doca_buf *dst_buff;			/**< Destination data buffer. */
	struct doca_buf const *src_buff;		/**< Source data buffer.
							  *  The source buffer must be from local memory.
							  *  Note: when using doca_buf linked list, the length of the
							  *  first data element in the source buffer
							  *  must be at least 4B. */
	uint64_t *output_chksum;			/**< Output checksum. If it is a compress job the
							  *  checksum calculated is of the src_buf.
							  *  If it is a decompress job the checksum result
 ...
 */
__DOCA_EXPERIMENTAL
doca_error_t doca_compress_get_max_buf_size(const struct doca_devinfo *devinfo,
					    enum doca_compress_job_types job_type, uint64_t *max_buffer_size);
