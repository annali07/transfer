 ...
 * Device must have export capability. See doca_devinfo_get_is_mmap_export_dpu_supported() in doca_dev.h
 ...
 */
__DOCA_EXPERIMENTAL
doca_error_t doca_mmap_export_dpu(struct doca_mmap *mmap, const struct doca_dev *dev, const void **export_desc,
 ...
 * In case the 'export_desc' was created using doca_mmap_export_dpu(), then device must have from export DPU capability.
 * See doca_devinfo_get_is_mmap_from_export_dpu_supported() in doca_dev.h
 * @param [out] mmap
 ...
 */
__DOCA_EXPERIMENTAL
doca_error_t doca_mmap_set_dmabuf_memrange(struct doca_mmap *mmap, int dmabuf_fd, size_t offset, size_t len);
