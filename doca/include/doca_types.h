 ...
 * DOCA_ACCESS_DPU_READ_ONLY and DOCA_ACCESS_DPU_READ_WRITE are mutually exclusive
...
enum doca_access_flags {
	DOCA_ACCESS_LOCAL_READ_ONLY	= 0,		/* Allows reading buffer by a local device but no write */
	DOCA_ACCESS_LOCAL_READ_WRITE	= (1 << 0),	/* Allows reading and writing to buffer by a local device */
	DOCA_ACCESS_RDMA_READ		= (1 << 1),	/* Allows reading buffer by a remote device using RDMA */
	DOCA_ACCESS_RDMA_WRITE		= (1 << 2),	/* Allows writing to buffer by a remote device using RDMA */
	DOCA_ACCESS_RDMA_ATOMIC		= (1 << 3),	/* Allows atomic operations by remote device using RDMA */
	DOCA_ACCESS_DPU_READ_ONLY	= (1 << 4),	/**
							 * Allows reading buffer by a DPU device but no write
							 * Only relevant for HOST - see doca_mmap_export_dpu()
							 */
	DOCA_ACCESS_DPU_READ_WRITE	= (1 << 5),	/**
							 * Allows reading and writing to buffer by a DPU device
							 * Only relevant for HOST - see doca_mmap_export_dpu()
							 */
};
 ...
 */
#ifdef __linux__
	typedef __be16 doca_be16_t;
	typedef __be32 doca_be32_t;
	typedef __be64 doca_be64_t;
#else /* Windows */
...
enum doca_gpu_mem_type {
	...
	/* CPU memory not accessible from GPU. */
	DOCA_GPU_MEM_CPU	= 2,
