			  ...
			  char *export_desc_file_path, char *buffer_info_file_path)
{
	...
	if (fp == NULL) {
		fprintf(stderr, "Failed to create the DMA copy file\n");
		return DOCA_ERROR_IO_FAILED;
	}

	if (fwrite(export_desc, 1, export_desc_len, fp) != export_desc_len) {
		fprintf(stderr, "Failed to write all data into the file\n");
		fclose(fp);
		return DOCA_ERROR_IO_FAILED;
	}
		     ...
		     char *export_desc_file_path, char *buffer_info_file_name)
{
	...
	fprintf("Please copy %s and %s to the DPU and run DMA Copy DPU sample before closing", export_desc_file_path, buffer_info_file_name);
...
main(int argc, char **argv)
{
	...
	strcpy(dma_conf.pci_address, "e1:00.2");
	strcpy(dma_conf.cpy_txt, "This is a sample piece of text");
	strcpy(dma_conf.export_desc_path, "/home/annali/exp.txt");
	strcpy(dma_conf.buf_info_path, "/home/annali/buf.txt");

	fprintf("Starting the sample");
	...
	if (src_buffer == NULL) {
		fprintf(stderr, "Source buffer allocation failed\n");
		goto sample_exit;
	}
	...
	if (result != DOCA_SUCCESS) {
		fprintf(stderr, "dma_copy_host() encountered an error: %s\n", doca_get_error_string(result));
	...
	if (exit_status == EXIT_SUCCESS)
		fprintf(stderr, "Sample finished successfully\n");
	else
		fprintf(stderr, "Sample finished with errors\n");
	return exit_status;
}
